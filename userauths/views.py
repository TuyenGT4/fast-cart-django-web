...
#         messages.warning(request, f"Bạn đã đăng nhập rồi")
#         return redirect('/')   
...
#     if request.user.is_authenticated:
#         messages.warning(request, "Ban đã đăng nhập rồi")
...
#             else:
#                 messages.error(request, 'Xác thực captcha không thành công. Vui lòng thử lại.')
        ...
        if form.is_valid():
            ...
            try:
                ...
                if user_instance is not None:
                    ...
                    messages.error(request, 'Tên người dùng hoặc mật khẩu không tồn tại')
            except userauths_models.User.DoesNotExist:
                messages.error(request, 'Người dùng không tồn tại')
