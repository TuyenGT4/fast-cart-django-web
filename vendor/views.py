...
def update_order_item_status(request, order_id, item_id):
    ...
    if request.method == "POST":
        ...
        messages.success(request, "Trạng thái sản phẩm đã được cập nhật")
...
def delete_coupon(request, id):
    coupon = store_models.Coupon.objects.get(vendor=request.user, id=id)
    coupon.delete()
    messages.success(request, "Mã giảm giá đã được xóa")
    return redirect("vendor:coupons")
...
def create_coupon(request):
    ...
    messages.success(request, "Mã giảm giá đã được tạo")
    return redirect("vendor:coupons")
...
def update_reply(request, id):
    ...
    messages.success(request, "Phản hồi đã được thêm")
...
def mark_noti_seen(request, id):
    ...
    messages.success(request, "Thông báo đã được đánh dấu đã xem")
...
def change_password(request):
    if request.method == "POST":
        ...
        if confirm_new_password != new_password:
            messages.error(request, "Mật khẩu xác nhận và mật khẩu mới không khớp")
        ...
        if check_password(old_password, request.user.password):
            ...
            messages.error(request, "Mât khẩu cũ không đúng")
            return redirect("vendor:change_password")
...
def delete_variants(request, product_id, variant_id):
    ...
    variants.delete()
    return JsonResponse({"message": "Tùy chọn sản phẩm đã được xóa thành công"})
...
def delete_variants_items(request, variant_id, item_id):
    ...
    item.delete()
    return JsonResponse({"message": "Giá trị tùy chọn đã được xóa thành công"})
...
def delete_product_image(request, product_id, image_id):
    ...
    image.delete()
    return JsonResponse({"message": "Ảnh sản phẩm đã được xóa thành công"})
