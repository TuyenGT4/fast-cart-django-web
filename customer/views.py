...
def add_to_wishlist(request, id):
    if request.user.is_authenticated:
        ...
        return JsonResponse({"message": "Sản phẩm đã được thêm vào danh sách yêu thích", "wishlist_count": wishlist.count()})
    else:
        return JsonResponse({"message": "Người dùng chưa đăng nhập", "wishlist_count": "0"})
...
def mark_noti_seen(request, id):
    ...
    messages.success(request, "Thống báo đã được đánh dấu là đã xem")
...
def address_create(request):
    if request.method == "POST":
        ...
        messages.success(request, "Địa chỉ đã được tạo")
...
def change_password(request):
    if request.method == "POST":
        ...
        if check_password(old_password, request.user.password):
            ...
            request.user.save()
            messages.success(request, "Mat khẩu đã được thay đổi")
            return redirect("customer:profile")
        else:
